...
from preregistration.data import Atlas, PatientCase, ensure_path
...
from preregistration.prereg_utils import (
    compute_dice,
    compute_init_transform,
    crop_images_to_mask,
    extract_slices,
    get_displacement_from_velocity,
    label_image_to_lung_mask,
    label_image_to_reg_mask,
    resample_image_to_2x2x2,
    undo_resample_image_to_2x2x2,
)
from preregistration.sitk_linear_registration import (
    sitk_inverse_linear_transform,
    sitk_register_affine,
    sitk_register_similarity,
    sitk_transform_ct_image,
    sitk_transform_mask_image,
)
...
def total_segmentation(patient: PatientCase, force=False, debug=True):
    ...
    if not patient.is_available("total_segm") or force:
        ...
        try:
            ...
            subprocess.run(
                cmd, shell=True, executable="/bin/bash", cwd="/homes/ykawamura"
            )
    ...
    if not patient.is_available("reg_mask") or force:
        ...
        if debug:
            ...
            sitk.WriteImage(
                reg_mask_slice,
                os.path.join(out_png_dir, f"{patient.patient_id}_reg_mask.png"),
            )
    if not patient.is_available("lung_mask") or force:
        ...
        if debug:
            ...
            sitk.WriteImage(
                reg_mask_slice,
                os.path.join(out_png_dir, f"{patient.patient_id}_lung_mask.png"),
            )
...
def linear_registration(patient: PatientCase, atlas: Atlas, force=False, debug=True):
    ...
    if (
        patient.is_available("transform_affine")
        and patient.is_available("transform_rigid")
        and not force
    ):
    ...
    if not patient.is_available("transform_rigid") or force:
        ...
        rigid_transform, moving_resampled = sitk_register_similarity(
            fixed_image=pat_reg_mask,
    ...
    affine_transform, moving_resampled = sitk_register_affine(
        fixed_image=pat_reg_mask,
        moving_image=atlas_reg_mask,
    ...
    if debug:  # not really needed
        ...
        pat_ct_affine = sitk_transform_ct_image(
            transform=inverse_affine_transform,
            fixed_image=atlas_reg_mask,
            moving_image=patient.get_image("ct"),
        )
        patient.set_image("ct_affine", pat_ct_affine, save_image=save_images)
        pat_reg_mask_affine = sitk_transform_mask_image(
            transform=inverse_affine_transform,
            fixed_image=atlas_reg_mask,
    ...
    if debug:
        ...
        sitk.WriteImage(
            pat_ct_affine_slice,
            os.path.join(out_png_dir, f"{patient.patient_id}_ct_affine.png"),
        )
...
def resample_images(patient: PatientCase, atlas: Atlas, force=False, debug=True):
    ...
    if (
        patient.is_available("ct_affine_resample")
        and patient.is_available("segm_affine_resample")
        and not force
    ):
    ...
    pat_to_atlas_transform = sitk_inverse_linear_transform(
        patient.get_image("transform_affine")
    )
    print("  transform ct and segm to atlas")
    pat_to_atlas_ct_image = sitk_transform_ct_image(
        pat_to_atlas_transform, fixed_image=atlas_reg_mask, moving_image=pat_ct_image
    )
    pat_to_atlas_segm_image = sitk_transform_mask_image(
        pat_to_atlas_transform, fixed_image=atlas_reg_mask, moving_image=pat_segm_image
    )
    print("  resample ct and segm to resolution 2x2x2")
    pat_to_atlas_ct_image_resampled = resample_image_to_2x2x2(
        pat_to_atlas_ct_image, interpolator=sitk.sitkLinear, default_value=-1024
    )
    pat_to_atlas_segm_image_resampled = resample_image_to_2x2x2(
        pat_to_atlas_segm_image, interpolator=sitk.sitkLabelGaussian, default_value=0
    )
    patient.set_image(
        "ct_affine_resample", pat_to_atlas_ct_image_resampled, save_image=save_images
    )
    patient.set_image(
        "segm_affine_resample",
        pat_to_atlas_segm_image_resampled,
        save_image=save_images,
    )
    if debug:
        out_png_dir = os.path.join(patient.png_dir, "ct_resample")
        ensure_path(out_png_dir)
        pat_ct_affine_slice = extract_slices(
            patient.get_image("ct_affine_resample"), axis="y"
        )
        sitk.WriteImage(
            pat_ct_affine_slice,
            os.path.join(out_png_dir, f"{patient.patient_id}_ct_affine_resample.png"),
        )
        pat_ct_affine_slice = extract_slices(
            patient.get_image("ct_affine_resample"), axis="x"
        )
        sitk.WriteImage(
            pat_ct_affine_slice,
            os.path.join(
                out_png_dir, f"sag_{patient.patient_id}_ct_affine_resample.png"
            ),
        )
    if debug:
        ...
        ensure_path(out_png_dir)
        pat_ct_affine_slice = extract_slices(
            patient.get_image("segm_affine_resample"), axis="y", is_ct=False
        )
        sitk.WriteImage(
            pat_ct_affine_slice,
            os.path.join(out_png_dir, f"{patient.patient_id}_segm_affine_resample.png"),
        )
        pat_ct_affine_slice = extract_slices(
            patient.get_image("segm_affine_resample"), axis="x", is_ct=False
        )
        sitk.WriteImage(
            pat_ct_affine_slice,
            os.path.join(
                out_png_dir, f"sag_{patient.patient_id}_segm_affine_resample.png"
            ),
        )


def variational_registration(
    patient: PatientCase, atlas: Atlas, device="cuda", force=False, debug=True
):
    print(f"RUN variational registration for patient id {patient.patient_id}")
    if (
        patient.is_available("to_atlas_displ")
        and patient.is_available("to_atlas_velo")
        and not force
    ):
        print(f"  SKIP variational registration because already done!")
        return
    atlas_segm_image = atlas.get_image(
        "segm_resample", as_pixel_type=sitk.sitkFloat32
    )  # fixed image
    pat_segm_affine_image = patient.get_image(
        "segm_affine_resample", as_pixel_type=sitk.sitkFloat32
    )  # moving image
    print("  run variational registration")
    output_varreg_dict = run_var_reg(
        sitk_fixed_image=atlas_segm_image,
        sitk_moving_image=pat_segm_affine_image,
        iterations=800,
        alpha=0.5,
        device=device,
    )
    patient.set_image(
        "to_atlas_displ", output_varreg_dict["displacement"], save_image=save_images
    )
    patient.set_image(
        "to_atlas_velo", output_varreg_dict["velocity"], save_image=save_images
    )
    print("  warp ct and segm images")
    displ_field = output_varreg_dict["displacement"]
    pat_segm_warped_image = warp_image_sitk(
        pat_segm_affine_image,
        displ_field,
        default_value=0,
        interpolator=sitk.sitkNearestNeighbor,
    )
    patient.set_image(
        "to_atlas_segm_warped", pat_segm_warped_image, save_image=save_images
    )
    output_varreg_dict["dice_before"] = compute_dice(
        atlas_segm_image, pat_segm_affine_image
    )
    output_varreg_dict["dice_after"] = compute_dice(
        atlas_segm_image, pat_segm_warped_image
    )
    output_varreg_dict["patid"] = patient.patient_id
    print(
        f"DICE scores of patient id {patient.patient_id}: before={output_varreg_dict['dice_before']} after={output_varreg_dict['dice_after']}"
    )
    if debug:
        pat_ct_affine_image = patient.get_image("ct_affine_resample")
        pat_ct_warped_image = warp_image_sitk(
            pat_ct_affine_image,
            displ_field,
            default_value=-1024,
    ...
    if debug:
        out_csv_file = patient.filenames["to_atlas_displ"].replace(
            "to_atlas_displ.nii.gz", "to_atlas_stats.csv"
        )
        ...
        ensure_path(out_png_dir)
        pat_ct_affine_slice = extract_slices(
            patient.get_image("to_atlas_ct_warped"), axis="y", is_ct=True
        )
        sitk.WriteImage(
            pat_ct_affine_slice,
            os.path.join(out_png_dir, f"{patient.patient_id}_to_atlas_ct_warped.png"),
        )
        pat_ct_affine_slice = extract_slices(
            patient.get_image("to_atlas_ct_warped"), axis="x", is_ct=True
        )
        sitk.WriteImage(
            pat_ct_affine_slice,
            os.path.join(
                out_png_dir, f"sag_{patient.patient_id}_to_atlas_ct_warped.png"
            ),
        )


def transform_atlas_maps_to_patient(
    patient: PatientCase, atlas: Atlas, force=False, debug=True
):
    ...
    atlas_to_pat_displ = get_displacement_from_velocity(
        pat_to_atlas_velocity, inverse=True
    )
    # print(f"{atlas_to_pat_displ.GetSize()} {atlas_to_pat_displ.GetSpacing()} {atlas_to_pat_displ.GetOrigin()}")
    orig_size_atlas_image = atlas.get_image(
        "total_segm"
    )  # faster to load (and needed anyway) as atlas.get_image('ct')
    atlas_to_pat_displ = undo_resample_image_to_2x2x2(
        atlas_to_pat_displ,
        sitk_reference_image=orig_size_atlas_image,
        interpolator=sitk.sitkLinear,
        default_value=0,
    )
    ...
    composite_transform.AddTransform(
        sitk.DisplacementFieldTransform(atlas_to_pat_displ)
    )
    ...
    patient_distance_map = sitk.VectorMagnitude(
        resampler.Execute(atlas.atlas_coordinate_map)
    )
    ...
    atlas_to_pat_avg_label_map = sitk.RecursiveGaussian(
        atlas_to_pat_avg_label_map, sigma=5.0
    )
    atlas_to_pat_avg_label_map = sitk.Cast(
        sitk.RescaleIntensity(
            atlas_to_pat_avg_label_map, outputMinimum=0.0, outputMaximum=255.0
        ),
...
def process_file(filename: str, patient_id: str):
    ...
    crop_ct_image, crop_dist_map, crop_prob_map = crop_images_to_mask(
        [ct_image, dist_map, prob_map], mask_image=lung_mask
    )
    ...
    patient.save_image(
        "lung_mask"
    )  # needed for undo cropping in post-processing (or save crop parameters somewhere)
    et = time.time()
