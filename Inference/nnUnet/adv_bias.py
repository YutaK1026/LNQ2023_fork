# code adapted from: https://github.com/cheng-01037/Causality-Medical-Image-Domain-Generalization/blob/main/biasfield_interpolate_cchen/adv_bias.py
...
from nnUnet.adv_transformation_base import AdvTransformBase
...
class AdvBias(AdvTransformBase):
    ...
    def init_parameters(self):
        ...
        self._image_size = np.array([self.data_size[2], self.data_size[3], self.data_size[4]])
...
https://github.com/airlab-unibas/airlab/blob/1a715766e17c812803624d95196092291fa2241d/airlab/transformation/pairwise.py
    ...
    def compute_smoothed_bias(self, cpoint=None,interpolation_kernel=None, padding=None,stride=None):
        ...
        if scale_factor>1:
            diff_bias = F.interpolate(bias_field_tmp, scale_factor=scale_factor, mode='trilinear', align_corners=True)
        else:
...
if __name__ == "__main__":
    ...
    blender_cofig = {
        ...
        'downscale': 2,  #
        'data_size': [1, 1, 56,128,224],
        'interpolation_order': 2,
        'init_mode': 'gaussian',
        'space': 'log'
    }
    augmentor = AdvBias(config_dict=blender_cofig, debug=True,use_gpu=True)
    augmentor.init_parameters()
    bias_field =  rescale_intensity(augmentor.bias_field)
    print("")
