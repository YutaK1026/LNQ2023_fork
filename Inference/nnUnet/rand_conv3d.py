...
'''
Adopted from Ouyang et al. "Causality-Inspired Single-Source Domain Generalization for Medical Image Segmentation"
https://github.com/cheng-01037/Causality-Medical-Image-Domain-Generalization/blob/main/models/imagefilter3d.py
'''
...
class GINGroupConv3D(nn.Module):
    def __init__(self, out_channel = 3, in_channel = 3, interm_channel = 2, scale_pool = [1, 3 ], n_layer = 4, out_norm = 'frob', init_scale = 'default', alpha_min=0, alpha_max=1.0):
        '''
        GIN
        '''
    ...
    def forward(self, x_in):
        ...
        alphas = ((self.alpha_min - self.alpha_max) * torch.rand(nb) + self.alpha_max)[:, None, None, None, None] # nb, 1, 1, 1, 1
...
if __name__ == '__main__':
    xin = torch.rand([5, 1, 64, 64, 64]).cuda()
    augmenter = GINGroupConv3D(in_channel=1, out_channel=10, n_layer=4, interm_channel=2).cuda()
    out = augmenter(xin)
