...
class Lnq2023(SegmentationAlgorithm):
    def __init__(self):
        ...
        self.predictor.initialize_from_trained_model_folder(
            model_training_output_dir=os.path.join(nnUNet_results_dir, 'Dataset017_LNQ2023/trainer_nnUNet_Plans_3d_fullres/'),
    ...
    def preprocess(self, idx: int, input_image_file_path: Path) -> torch.Tensor:
        ...
        probm_path = os.path.join(nnUNet_raw_dir, f'ct_{idx+1:03d}', f'ct_{idx+1:03d}_0002.nii.gz')
        prediction, properties = self.predictor.predict_from_files(str(os.path.join(nnUNet_raw_dir, f'ct_{idx + 1:03d}')),
