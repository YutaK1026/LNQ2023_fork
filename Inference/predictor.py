...
class predictor(nnUNetPredictor):
    ...
    def __init__(self,
                 ...
                 allow_tqdm: bool = True, ensemble_method: str = 'avg'):
        super().__init__(tile_step_size=tile_step_size, use_gaussian=use_gaussian, use_mirroring=use_mirroring,
                         perform_everything_on_gpu= perform_everything_on_gpu, device=device, verbose=verbose,
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        network = trainer.build_network_architecture(plans_manager, dataset_json, configuration_manager,
                ...
                try:
                    for params in self.list_of_parameters:
                        ...
                        if prediction is None:
                            ...
                            list_predictions.append(self.predict_sliding_window_return_logits(data))
    ...
    def predict_from_files(self,
        ...
        data, seg, properties = preprocessor.run_case(list_of_lists_or_source_folder[0],
                                                           None,
                                                           self.plans_manager,
        ...
        if type(prediction) == list:
            ...
            for pred in prediction:
                pred = pred.cpu()
                prediction_converted = convert_predicted_logits_to_segmentation_with_correct_shape(pred,
                                                                                                   self.plans_manager,
                                                                                                   self.configuration_manager,
                                                                                                   self.label_manager,
                                                                                                   properties,
            ...
            prediction_converted = convert_predicted_logits_to_segmentation_with_correct_shape(prediction,
                                                                                               self.plans_manager,
                                                                                               self.configuration_manager,
                                                                                               self.label_manager,
                                                                                               properties,
